...
import { PiperxswapConfig } from './piperxswap/piperxswap.config';
    ...
    asyncHandler(async (_req, res: Response<ConnectorsResponse, {}>) => {
      res.status(200).json({
        connectors: [
          ...
          {
            name: 'piperxswap',
            trading_type: PiperxswapConfig.config.tradingTypes,
            chain_type: PiperxswapConfig.config.chainType,
            available_networks: PiperxswapConfig.config.availableNetworks,
          },
        ],
      });
    })
