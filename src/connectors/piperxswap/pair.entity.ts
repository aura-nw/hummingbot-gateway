...
export class Pair {
  ...
  public getOutputAmount(
    ...
    const denominator = JSBI.add(
      JSBI.multiply(inputReserve.quotient, JSBI.BigInt(1000)),
      inputAmountWithFee,
    );
    ...
    if (JSBI.equal(outputAmount.quotient, JSBI.BigInt(1000))) {
  ...
  public getLiquidityValue(
    ...
    if (!feeOn) {
      ...
      if (!JSBI.equal(kLastParsed, JSBI.BigInt(0))) {
        const rootK = sqrt(
        ...
        if (JSBI.greaterThan(rootK, rootKLast)) {
          ...
          const denominator = JSBI.add(
            JSBI.multiply(rootK, JSBI.BigInt(5)),
            rootKLast,
          );
